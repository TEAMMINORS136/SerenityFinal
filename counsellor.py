...
from PyQt5.QtWidgets import (
    ...
    QTableWidget, QTableWidgetItem, QHeaderView, QHBoxLayout, QScrollArea, QFrame, QDateEdit, QFormLayout
)
...
def insert_dummy_data():
    counsellors = [
        ("Dr. John Doe", 45, "john.doe@example.com", "GMT+1", "Experienced in anxiety and depression.", "Anxiety", "10:00 AM,2:00 PM"),
        ("Dr. Jane Smith", 38, "jane.smith@example.com", "GMT+2", "Specializes in relationship counseling.", "Relationships", "4:00 PM,6:00 PM"),
        ("Dr. Emily Brown", 50, "emily.brown@example.com", "GMT+0", "Expert in trauma and PTSD.", "Trauma", "10:00 AM,6:00 PM"),
        ("Dr. Michael Lee", 42, "michael.lee@example.com", "GMT-5", "Focuses on career counseling.", "Career", "2:00 PM,4:00 PM"),
        ("Dr. Sarah Green", 55, "sarah.green@example.com", "GMT+3", "Specializes in child psychology.", "Child Psychology", "10:00 AM,4:00 PM")
    ]
    conn = sqlite3.connect("counsellors.db")
    c = conn.cursor()
    c.executemany('''INSERT INTO counsellors (name, age, email, timezone, profile, specialty, availability)
                     VALUES (?, ?, ?, ?, ?, ?, ?)''', counsellors)
    conn.commit()
    conn.close()

# Fetch all counsellors from the database
def fetch_counsellors():
    conn = sqlite3.connect("counsellors.db")
    c = conn.cursor()
...
def save_booking(counsellor_name, user_email, date, time_slot, message):
    conn = sqlite3.connect("counsellors.db")
    c = conn.cursor()
    c.execute('''INSERT INTO bookings (counsellor_name, user_email, date, time_slot, message)
                 VALUES (?, ?, ?, ?, ?)''', (counsellor_name, user_email, date, time_slot, message))
    conn.commit()
    conn.close()
...
class CounsellorApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.bookings_table.setMaximumHeight(150) 
        self.bookings_table.setHorizontalHeaderLabels(["Counsellor", "User Email", "Date", "Time Slot", "Message"])
        ...
        self.counsellors = fetch_counsellors()
    ...
    def book_session(self, counsellor):
        ...
        self.time_input.addItems(counsellor[7].split(","))  # Use counsellor's availability
    ...
    def confirm_booking(self, counsellor):
        ...
        if not message:
            QMessageBox.warning(self, "Error", "Please enter a message for the counsellor.")
            return

        # Save booking to database
        user_email = "user@example.com"  # Replace with user's email
        save_booking(counsellor[1], user_email, date, time_slot, message)
...
Your session with {counsellor[1]} has been booked for {date} at {time_slot}.
...
You have a new session booked for {date} at {time_slot}.
...
if __name__ == "__main__":
    ...
    window = CounsellorApp(None, None)
    window.show()
    sys.exit(app.exec_())